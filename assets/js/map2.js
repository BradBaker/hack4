  ...
  d3.json("/map_jsons/world-110m.json", function(error, world) {
    ...
    // Load user data.
    d3.json("/data/bbs_data2.json", function(error,data) {


// add in filters      
      xxx = []
      for(var key in data.friends_trips){
        xxx.push(data.friends_trips[key].location)
      }
      top_cities = _.uniq(xxx)
      ...
      function update_data() {
        $(".my_trips").remove()
        $(".personal_points").remove()
        $(".friends_trips").remove()
        $(".friend_routes").remove()

        var include_locations = new Array();
        $("input:checked").map(function(){ include_locations.push($(this).attr("name"))});
     ...
     my_route_plotted = svg.append("g")
      ...
      var friends_destinations = [];
        ...
        for (var key in data.friends_trips) {
             if(include_locations.indexOf(data.friends_trips[key].location)>= 0){
